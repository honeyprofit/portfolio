$(document).ready(function () {
  let projectData = {
    younghyunkim: {
      ...
      description:
        "이 프로젝트는 제 포트폴리오 사이트입니다. 다크 모드와 라이트 모드를 성공적으로 사용하였습니다. SVG 로고를 활용하여 이미지 수를 줄이고 코드를 간결하게 유지하였으며, 여러 프로젝트를 효율적으로 동일한 형식으로 표현하기 위해 데이터를 객체 형식으로 저장하고, 이를 projectID를 통해 접근하는 방식을 사용했습니다.",
  ...
  if (projectData.hasOwnProperty(projectID)) {
    ...
    $("#project-image6").attr("src", projectData[projectID].image6);
    $("#project-title").text(projectData[projectID].title);
    $("#project-description").html(projectData[projectID].description);
    ...
    $("#summary-final a, #stamp_preview").attr(
      "href",
      projectData[projectID].link
    );
    $("#project-challenge").html(projectData[projectID].challenge);
    $("#project-goal").html(projectData[projectID].goals);
    $("#project-roles").html(projectData[projectID].roleandres);
    ...
    $("#project-value").html(projectData[projectID].value);
